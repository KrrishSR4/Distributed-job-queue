...
func TestWorkerPoolStartup(t *testing.T) {
	...
	pool := NewWorkerPool(5, "test-instance", repo, queue, processor, 1*time.Millisecond, 2*time.Millisecond, 30*time.Second, nil)
...
func TestWorkerJobLifecycleSuccess(t *testing.T) {
	...
	pool := NewWorkerPool(2, "test-instance", repo, queue, processor, 1*time.Millisecond, 2*time.Millisecond, 30*time.Second, nil)
	pool.Start()
...
func TestWorkerJobLifecycleFailure(t *testing.T) {
	...
	pool := NewWorkerPool(1, "test-instance", repo, queue, failingProcessor, 1*time.Millisecond, 2*time.Millisecond, 30*time.Second, nil)
	pool.Start()
...
func TestWorkerPoolConcurrency(t *testing.T) {
	...
	// 3 workers
	pool := NewWorkerPool(3, "test-instance", repo, queue, proc, 1*time.Millisecond, 2*time.Millisecond, 30*time.Second, nil)
	pool.Start()
...
func TestWorkerPoolGracefulShutdown(t *testing.T) {
	...
	pool := NewWorkerPool(3, "test-instance", repo, queue, processor, 1*time.Millisecond, 2*time.Millisecond, 30*time.Second, nil)
	pool.Start()
